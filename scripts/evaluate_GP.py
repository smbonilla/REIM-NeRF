...
def main(args):
    ...
    dataset_root_dir = sorted(list(Path(args.dataset_root_dir).iterdir()))
    results_root_dir = Path(args.predictions_root_dir)

    eval_h, eval_w = (240, 320)
    ...
    for dataset in tqdm(dataset_root_dir, desc='datasets:'):
        ...
        # images are inside ds_root_dir/images/ and distmaps are inside ds_root_dir/distmaps/

        # if images are stored as "1_color.png"
        if ref_img_paths[0].name.split('_')[0].isdigit():
            predicted_img_paths = [ds_root_dir/f"{int(p.name.split('_')[0])-1:03d}.png" for p in ref_img_paths]
            predicted_distmap_paths = [ds_root_dir/f"depth_{int(p.name.split('_')[0])-1:03d}.pfm" for p in ref_img_paths]
        else: # images are stored as "219.png" - "589.png" so must sort ref_img_paths then get predicted_img_paths as the idx of enumerate(ref_img_paths) - 1 (since the first image is 0.png and not 1.png
            sorted_ref_img_paths = sorted(ref_img_paths, key=lambda x: int(x.name.split('.')[0]))
            predicted_img_paths = [ds_root_dir/f"{idx:03d}.png" for idx, p in enumerate(sorted_ref_img_paths)]
            predicted_distmap_paths = [ds_root_dir/f"depth_{idx:03d}.pfm" for idx, p in enumerate(sorted_ref_img_paths)]
        ...
        # before converting check if images fail to load
        pred_images_list = []
        for p in predicted_img_paths:
            img = cv2.imread(str(p))
            if img is None:
                raise Exception(f"Failed to load image: {p}")
            pred_images_list.append(img.astype(np.float32) / 255.0)

        # Ensure all images are loaded before proceeding
        if len(pred_images_list) != len(predicted_img_paths):
            print(f"Error: Not all images were loaded successfully.")
        ...
        np.savetxt(ds_root_dir/'30000_per_frame_psnr_ssim_dmse_lpips.csv', results, delimiter=",")
         
        # For aggregated results, include mean and std of LPIPS
        agg = np.array([[psnr_avg.numpy(), ssim_avg.numpy(), depth_mse_avg.numpy(), lpips_mean.numpy()],
                        [psnr_std.numpy(), ssim_std.numpy(), depth_mse_std.numpy(), lpips_std.numpy()]])
        np.savetxt(ds_root_dir/'30000_aggregated_psnr_ssim_dmse_lpips_mean_std.csv', agg, delimiter=",")

        results_root_dir_str = str(results_root_dir)
        if results_root_dir_str not in agg_dict:
            agg_dict[results_root_dir_str] = []
        agg_dict[results_root_dir_str].append(agg)
    ...
    print('model \t psnr\t ssim\tdmse\tlpips')
    for k, v in agg_dict.items():
        # v should be a list of 2d matrices of aggs
        method_results = np.array(v)  # dataset x stats x metric

        method_avg = np.mean(method_results, axis=0)
        print(f'{k} | {method_avg[0,0]:.03f}+-({method_avg[1,0]:.03f}) |\t {method_avg[0,1]:.03f} +- ({method_avg[1,1]:.03f}) |\t{method_avg[0,2]:.03f}+-({method_avg[1,2]:.03f})|\t{method_avg[0,3]:.03f}+-({method_avg[1,3]:.03f})')

        np.savetxt(results_root_dir/f'{k}_30000_psnr_ssim_dmse_lpips.csv', method_avg, delimiter=',')
